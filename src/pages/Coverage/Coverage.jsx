...
// Districts data (only a few shown for example)
const districtsData = [
  {
    region: "Dhaka",
    district: "Dhaka",
...
function MoveToDistrict({ position }) {
  const map = useMap();
  if (position) {
...
export default function CoverageMap() {
  ...
  const handleSearch = (e) => {
    ...
    const matches = districtsData.filter((d) =>
      d.district.toLowerCase().includes(term.toLowerCase())
    );
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6 p-6 bg-green-50 min-h-screen">
      <h1 className="text-4xl font-extrabold text-green-700 drop-shadow-md text-center">
        NGOConnect Coverage Across Bangladesh
      </h1>
      ...
      <div className="relative w-full max-w-md">
        <input
          ...
          className="w-full p-3 rounded-lg border-2 border-green-300 focus:outline-none focus:border-green-500 shadow"
        />

        {/* Suggestions dropdown */}
        {suggestions.length > 0 && (
          <ul className="absolute top-full left-0 w-full bg-white border border-green-300 rounded-b-lg max-h-52 overflow-auto z-50 shadow-md">
      ...
      {/* Map */}
      <div className="w-full max-w-6xl h-[600px] rounded-3xl overflow-hidden shadow-xl border-2 border-green-200">
        <MapContainer
        ...
        >
          ...
          {districtsData.map((district, index) => (
            <Marker
              ...
              icon={selectedDistrict === district ? selectedIcon : defaultIcon}
            >
              <Popup className="text-green-700 font-semibold">
                ...
                <p>
                  ...
                  {district.covered_area?.join(", ") || "Not available"}
                </p>
                <p>
                  <a
